# -*- coding: utf-8 -*-
# @Time    : 2021/1/11 19:51
# @Author  : wonderstone
# @FileName: Robot4file.py
# @Software: PyCharm
# @Ref     :
...
def get_proxies(filedir):
    ...
    if ss_nodes_list:
        decode_proxy = decode_ss_node(ss_nodes_list)
        clash_node = ss_to_clash(decode_proxy)
        proxy_list['proxy_list'].extend(clash_node['proxy_list'])
        proxy_list['proxy_names'].extend(clash_node['proxy_names'])
    if ssr_nodes_list:
        decode_proxy = decode_ssr_node(ssr_nodes_list)
